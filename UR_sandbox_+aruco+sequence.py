...
robot_startposition = (math.radians(-45.84),
                    math.radians(-91.21),
                    math.radians(138),
                    math.radians(-44.30),
                    math.radians(46.07),
...
def set_lookorigin():
    ...
    It describes the midpoint of the plane in which the
     follows faces.
    ...
    while True:
        if(step==0): #move to tag
            ...
            if xyoffset is not None:
                ...
                if (xyoffset[0] < 2 and xyoffset[0] > -2 and xyoffset[1] < 2 and xyoffset[1] > -2):
                    print("move done")
                    step = step + 1
            else:
                print("prout")
            ...
            qnear[1] += 0.04
            qnear[2] -= 0.01
            robot.movel(qnear, a=ACCELERATION, v=VELOCITY)
            print("move to certer of tag")
            time.sleep(6)
            step = step + 1
        elif(step==2):
            print("move back to home position")
            robot.movej(q=robot_startposition, a=ACCELERATION, v=VELOCITY)
            time.sleep(6)
